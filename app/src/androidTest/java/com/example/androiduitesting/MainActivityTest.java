...
public class MainActivityTest {
    ...
    public void testActivitySwitched(){
        ...
        // If activity switched, new stuff should be visible
        onView(withId(R.id.text_city_name)).check(matches(isDisplayed()));
        onView(withId(R.id.button_back)).check(matches(isDisplayed()));
    }
    @Test
    public void testCityNameIsRight(){
        // add to list
        onView(withId(R.id.button_add)).perform(click());
        onView(withId(R.id.editText_name)).perform(ViewActions.typeText("Edmonton"));
        onView(withId(R.id.button_confirm)).perform(click());
        // Click the first city
        onData(anything())
                .inAdapterView(withId(R.id.city_list))
                .atPosition(0)
                .perform(click());
        // Check the displayed city name matches
    ...
    public void testBackButton() {
        ...
        // Press back button on ShowActivity
